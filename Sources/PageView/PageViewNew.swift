                            ...
                            if (self.pageArrayViews.count > 1){
                                ...
                                VStack(alignment: .center) {
                                    PageControl(defaultImage: Image(systemName: "circle")
                                                                       .resizable()
                                        , selectedImage: Image(systemName: "circle.fill").resizable(), count: self.pageArrayViews.count, curPage: self.curPage).offset(y:self.height * 0.9 )
                                    Spacer()
    ...
    public var body: some View {
        HStack(alignment: .center){
            ...
            ForEach(0..<count) { i in
                self.curPage == i ? self.selectedImage.frame(width:10, height:10).foregroundColor(.black) :  self.defaultImage.frame(width:10, height:10).foregroundColor(.black)
            }
             Spacer()
        }
    }
